...
def main() -> None:
    p = argparse.ArgumentParser(description="Build report assets for the u300 representative50 scaffold validation.")
    ...
    p.add_argument("--metrics-dir", required=True, help="Directory containing method metrics CSVs.")
    p.add_argument("--pred-root", required=True, help="Predictions root directory containing per-method subdirs.")
    args = p.parse_args()
    ...
    methods = [
        MethodSpec(
            "rn_rs",
            "RNss(RS)",
            metrics_dir / "rnanneal_ss_rs_scaff.csv",
            pred_root / "rnanneal_ss_rs_scaff",
            "#1f77b4",
            "-",
        ),
        MethodSpec(
            "rn_lf",
            "RNss(LF)",
            metrics_dir / "rnanneal_ss_lf_scaff.csv",
            pred_root / "rnanneal_ss_lf_scaff",
            "#9467bd",
            "-",
        ),
        MethodSpec(
            "rn_ef",
            "RNss(EF)",
            metrics_dir / "rnanneal_ss_ef_scaff.csv",
            pred_root / "rnanneal_ss_ef_scaff",
            "#d62728",
            "-",
        ),
        MethodSpec("rnastructure", "RNAstr", metrics_dir / "rnastructure.csv", pred_root / "rnastructure", "#7f7f7f"),
        MethodSpec("linearfold", "LF-V", metrics_dir / "linearfold.csv", pred_root / "linearfold", "#ff7f0e"),
        MethodSpec("eternafold", "EFold", metrics_dir / "eternafold.csv", pred_root / "eternafold", "#2ca02c"),
    ]
    ...
    bucket_map = {
        str(row["target_id"]): str(row.get("bucket", "unknown")) for _, row in manifest_df.iterrows()
    }
    ...
    write_survival_tex(
        out_tex=gen / "survival_best100.tex", title="Best-of-100 (F1@100)", dat_prefix="survival_best100"
    )
    ...
    for bucket in buckets:
        bucket_ids = [tid for tid in sorted(shared) if bucket_map.get(tid, "unknown") == bucket]
        for m in methods:
    ...
    def write_boxplot_group_tex(*, out_tex: Path, metric: str, title: str) -> None:
        lines: list[str] = []
        lines.append(r"\begin{tikzpicture}")
        lines.extend(define_colors())
        lines.append(r"\begin{groupplot}[")
        lines.append(r"  group style={group size=3 by 2, horizontal sep=1.2em, vertical sep=1.2em},")
        lines.append(r"  width=0.33\linewidth, height=0.33\linewidth,")
        lines.append(r"  xmin=0.5, xmax=6.5,")
        lines.append(r"  ymin=0, ymax=1,")
        lines.append(r"  ymajorgrids=true, grid style={black!10},")
        lines.append(r"  xtick={1,2,3,4,5,6},")
        lines.append(r"  xticklabels={RNss(RS),RNss(LF),RNss(EF),RNAstr,LF-V,EFold},")
    ...
    write_boxplot_group_tex(out_tex=gen / "dist_f1_top1_by_bucket.tex", metric="f1_1", title="F1@1 distributions by bucket")
    write_boxplot_group_tex(
        out_tex=gen / "dist_f1_best100_by_bucket.tex",
        metric="f1_100",
        title="F1@100 distributions by bucket",
    )
