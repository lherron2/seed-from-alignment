...
def compute_summary(df: pd.DataFrame) -> dict[str, float]:
    ...
    out["solved70_best100"] = float((df["best_of_k_f1"] >= 0.7).mean())
...
def render_table_tex(*, out_path: Path, caption: str, label: str, headers: list[str], rows: list[list[str]]) -> None:
    lines: list[str] = []
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate LaTeX tables for FR3D under400 baseline comparison.")
    ...
    parser.add_argument("--pipeline", required=True, help="CSV from ssbench score for the main pipeline")
    parser.add_argument("--rnastructure", required=True, help="CSV from ssbench score for RNAstructure AllSub baseline")
    parser.add_argument("--linearfold", required=True, help="CSV from ssbench score for LinearFold Zuker baseline")
    ...
    methods = [
        MethodRow("pipeline", "RNAnneal-ss (pipeline)", Path(args.pipeline)),
        MethodRow("rnastructure", "RNAstructure (Fold MFE)", Path(args.rnastructure)),
        MethodRow("linearfold", "LinearFold-V (Zuker)", Path(args.linearfold)),
        MethodRow("eternafold", "EternaFold (MEA + samples)", Path(args.eternafold)),
    ]
    ...
    # Length bucket breakdown (computed from truth sequence lengths).
    ...
    for b in bucket_order:
        # Compute N once per bucket (shared across methods after alignment).
